...
// Wait for DOM to exist before touching the page
document.addEventListener("DOMContentLoaded", () => {
  const addrEl = document.getElementById("address");
  if (addrEl) addrEl.textContent = BTC_ADDRESS;
...
// Converts numbers into mining unit lingo (K, M, G, T, P, E)
function formatMiningNumber(value) {
  ...
  if (abs >= 1e18) { unit = "E"; divisor = 1e18; }
  else if (abs >= 1e15) { unit = "P"; divisor = 1e15; }
  else if (abs >= 1e12) { unit = "T"; divisor = 1e12; }
  else if (abs >= 1e9) { unit = "G"; divisor = 1e9; }
...
// ========================================================
// Card Builder (with optional progress bar)
// ========================================================
...
async function loadStats() {
  ...
  try {
    ...
    const btcDiffData = btcDiffRes?.ok ? await btcDiffRes.json() : null;
    const bchDiffData = bchDiffRes?.ok ? await bchDiffRes.json() : null;
    ...
    // 1) Current Session vs All-Time Best
    const sessionFraction =
      bestShareAllTime > 0 ? bestShareSession / bestShareAllTime : 0;

    shareEl.appendChild(
      createCard(
        "Best Share (Current Session)",
        formatMiningNumber(bestShareSession),
        `Raw: ${bestShareSession.toLocaleString()}`,
        bestShareAllTime
          ? {
              fraction: sessionFraction,
              label: `${(sessionFraction * 100).toFixed(2)}% of your all-time best`,
    ...
    // 2) All-Time Best vs BTC Difficulty
    let diffFraction = 0;
    let diffLabel = "";
    if (btcDifficulty && bestShareAllTime > 0) {
      diffFraction = bestShareAllTime / btcDifficulty;
      diffLabel = `${(diffFraction * 100).toPrecision(3)}% of BTC difficulty`;
    }

    shareEl.appendChild(
      createCard(
        "Best Share (All Time)",
        formatMiningNumber(bestShareAllTime),
    ...
    Object.entries(hrLabels).forEach(([field, label]) => {
      ...
      if (!val) return;
      hashrateEl.appendChild(createCard(label, val, "reported by CKPool"));
    });
        ...
        card.innerHTML = `
          ...
          <div class="grid grid-cols-2 gap-2 text-sm mt-2">
            ...
            <div>
              ...
              <p class="font-semibold">${w.hashrate7d || "—"}</p>
            </div>
          </div>

          <div class="mt-3 text-xs text-slate-400">
            <p>Last share:
              <span class="text-slate-200">${timeSince(w.lastshare)} (${formatTimestamp(w.lastshare)})</span>
            </p>
            <p>Session shares:
              <span class="text-slate-200">${w.shares.toLocaleString()}</span>
            </p>
            <p>Best share:
              <span class="text-slate-200">${w.bestshare.toLocaleString()}</span>
            </p>
          </div>
        `;
